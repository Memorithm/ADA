...
| ADA-A4 | Exact Entmax branch-and-bound | CPU-E0-CORRECTNESS / E1-QK-BOX-CANDIDATE |
...
- `E1-QK-BOX-CANDIDATE` means A4 has an isolated follow-on candidate that derives conservative page score bounds from precomputed coordinate-wise key min/max metadata and a query, then feeds those bounds into the unchanged E0 branch-and-bound controller. It is not correctness-qualified until the E1 local gates run green.
